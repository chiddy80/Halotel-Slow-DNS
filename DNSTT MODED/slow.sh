...
#define VIDEO_TTL 600
#define NORMAL_TTL 60
...
static request_t reqmap[65536];

static inline uint32_t dns_hash(unsigned char *b, int l){
    uint32_t h=5381;
    for(int i=12;i<l;i++) h=((h<<5)+h)^b[i];
    return h;
}

int is_video(unsigned char *b){
    return strstr((char*)b,"googlevideo") ||
           strstr((char*)b,"youtube") ||
           strstr((char*)b,"netflix") ||
           strstr((char*)b,"tiktok");
}

int cache_lookup(unsigned char *q,int ql,struct sockaddr_in *c,unsigned char *out){
    uint32_t h=dns_hash(q,ql);
    cache_t *e=&cache[h%CACHE_SIZE];
    if(e->hash==h && memcmp(&e->client,c,sizeof(*c))==0 && time(NULL)<e->expire){
...
void cache_store(unsigned char *q,int ql,struct sockaddr_in *c,unsigned char *r,int rl){
    uint32_t h=dns_hash(q,ql);
    cache_t *e=&cache[h%CACHE_SIZE];
    e->hash=h;
    e->client=*c;
    e->len=rl;
    memcpy(e->data,r,rl);
    e->expire=time(NULL)+(is_video(q)?VIDEO_TTL:NORMAL_TTL);
}

void patch_edns(unsigned char *b,int s){
    b[10]=0; b[11]=1;
    int p=12;
    while(b[p]) p++;
    p+=5;
    b[p]=0; b[p+1]=0; b[p+2]=41;
    b[p+3]=s>>8; b[p+4]=s&255;
}

int setnb(int f){ return fcntl(f,F_SETFL,fcntl(f,F_GETFL)|O_NONBLOCK); }

int main(){
    printf("[EDNS] Flow-locked streaming proxy active\n");
    ...
    struct sockaddr_in a={.sin_family=AF_INET,.sin_port=htons(LISTEN_PORT),.sin_addr.s_addr=INADDR_ANY};
