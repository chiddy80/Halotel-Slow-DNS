...
// ---------------- CONFIG ----------------
const (
	LISTEN_ADDR        = ":53"      // EDNS Proxy listen port
	UPSTREAM_ADDR      = "127.0.0.1:5300" // Upstream DNS (SlowDNS)
	EXTERNAL_EDNS      = 512
	INTERNAL_EDNS      = 1800
...
// ---------------- WORKER ----------------
func worker(wg *sync.WaitGroup, in <-chan packet, listener *net.UDPConn, upstream *net.UDPConn) {
	defer wg.Done()
